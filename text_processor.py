...
with log_exceptions(logger):
    logger.info("preparing text processing stuff..")
    ...
    logger.info("text processing is ready")
...
def find_in_text(text, categories):
    return [i for i, label in enumerate(categories) if label in text]
...
def find_categories_in_text(text):
    ...
    categories = set(find_in_text(text, categories_normalized)) | \
                 set(find_in_text(text, rus_categories_normalized))
